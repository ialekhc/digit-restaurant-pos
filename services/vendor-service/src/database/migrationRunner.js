...
const historyTable = 'vendor_schema_migrations';
...
export const migrate = async () => {
  ...
  const executed = new Set((await query(`SELECT migration_name FROM ${historyTable}`)).rows.map((row) => row.migration_name));

  for (const migration of migrations) {
    if (executed.has(migration.name)) continue;
    await withTransaction(async (client) => {
      await migration.up(client);
      await client.query(`INSERT INTO ${historyTable} (migration_name) VALUES ($1)`, [migration.name]);
    });
    console.log(`[vendor-db] migrated ${migration.name}`);
  }
};

export const rollback = async () => {
  await withTransaction(async (client) => ensureHistory(client));
  const { rows } = await query(`SELECT migration_name FROM ${historyTable} ORDER BY id DESC LIMIT 1`);
  if (!rows.length) {
    console.log('[vendor-db] no migrations to rollback');
    return;
  }
  ...
  await withTransaction(async (client) => {
    ...
    await client.query(`DELETE FROM ${historyTable} WHERE migration_name = $1`, [migration.name]);
  });
  console.log(`[vendor-db] rolled back ${migration.name}`);
};
...
export const status = async () => {
  ...
  const migrations = await loadMigrations();
  const executed = new Set((await query(`SELECT migration_name FROM ${historyTable}`)).rows.map((row) => row.migration_name));
  migrations.forEach((migration) => console.log(`${executed.has(migration.name) ? 'up' : 'down'} ${migration.name}`));
};
