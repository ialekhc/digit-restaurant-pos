  ...
  return (
    <label className="block">
      ...
      <select
        ref={ref}
        {...props}
        className={`w-full rounded-xl border border-brand-100 bg-white px-3.5 py-2.5 text-sm outline-none transition focus:border-brand-500 focus:ring-4 focus:ring-brand-100 ${className}`}
      >
        {options.map((option) => (
