...
const defaultApiBaseUrl = 'http://127.0.0.1:5500/api';
const apiBaseUrl = process.env.DIGIT_DESKTOP_API_BASE_URL || process.env.VITE_API_URL || defaultApiBaseUrl;
const apiHealthUrl = `${apiBaseUrl.replace(/\/+$/, '')}/health`;
const localApiUrl = new URL(apiBaseUrl);
const backendPort = localApiUrl.port || (localApiUrl.protocol === 'https:' ? '443' : '80');
const shouldUseLocalBackend =
  ['localhost', '127.0.0.1'].includes(localApiUrl.hostname) &&
...
const startBackend = async () => {
  ...
  backendProcess.stdout.on('data', (chunk) => {
    console.log(`[desktop-backend] ${chunk.toString().trim()}`);
  });
